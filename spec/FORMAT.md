...
## Canonical JSON（哈希域规范化）

> 本节是规范中**唯一**规定字节序列的部分。其余章节描述"有哪些字段"，
> 本节描述"这些字段如何变成字节"。

AILog 的完整性依赖一条不变式：**同一份记录 → 同一个哈希**。
凡是要进入**哈希域**（Merkle leaf、anchor、proof bundle）的记录，必须先序列化为
**Canonical JSON（CJSON）**。

### 为什么必须单独立规

`.ailog` 是 JSONL，字段顺序在 JSON 语义上无意义，但 `sha256` 只认字节。
若两个实现各按宿主语言的习惯序列化，同一份记录会得到**不同哈希**，锚点会被判成
`tampered`——这是**假阴性**，比漏检更伤信任。

因此：**实现不得依赖宿主语言的默认 JSON 序列化。**
`JSON.stringify()` 与 `json.dumps()` 的默认行为都不满足本节，且两者互不相同
（前者按插入顺序输出，后者同样只在显式排序时才有序）。

### 规则

记录 `v` 被序列化为 UTF-8 字节串 `B`：

| # | 规则 |
|---|------|
| **C1** | 编码 UTF-8，无 BOM，无尾随换行 |
| **C2** | 对象键**递归**按 Unicode 码点升序排列；数组元素顺序**保持**，不排序 |
| **C3** | 无多余空白。分隔符为 `,` 与 `:`，冒号后不加空格 |
| **C4** | 仅转义 `"`、`\` 与 U+0000–U+001F。其中 U+0008/0009/000A/000C/000D 用 `\b` `\t` `\n` `\f` `\r` 短转义，其余控制字符用 `\u00XX`（小写十六进制）。**所有非 ASCII 字符按原样输出 UTF-8，不得转义为 `\uXXXX`** |
| **C5** | 数字**只允许整数**，且满足 \|n\| ≤ 2^53 − 1。浮点数、`NaN`、`Infinity` 一律非法 |
| **C6** | 没有值的可选字段**直接省略**，不得写 `null` 占位。`null` 仅用于语义上真正的空值 |
| **C7** | 同一对象内**禁止重复键**；校验方应当拒绝此类记录 |

**关于 C2**：必须是**码点序**，不是 UTF-16 码元序。多数语言默认按键的 UTF-16 顺序
排序，遇到 U+10000 以上的字符会给出与本节相反的结果。参考实现按 UTF-8 字节序比较
——UTF-8 字节序与码点序等价。

**关于 C5**：这是对**数据模型**的约束，不只针对序列化器。
理由很实际：`1.0` 在 Python 序列化为 `1.0`，在 JavaScript 序列化为 `1`，跨语言不可调和。
把浮点挡在哈希域之外，比试图统一浮点格式便宜得多。

### 参考实现

| 语言 | 位置 | 入口 |
|------|------|------|
| Python | `ailog/core/canonical.py` | `canonical_json()` / `canonical_sha256()` |
| JavaScript | `local-chain/packages/canonical` | `canonicalJson()` / `canonicalSha256()` |
| JavaScript | `evidence/modules/canonical.js` | 同上 |

### 测试向量

`spec/canonical-vectors.json` 是**跨实现的一致性契约**。

其中 `canonical` 字段是**按本节规则手工写定**的期望输出（不是由任何实现生成的），
`sha256` 为其 UTF-8 字节的 SHA-256。每个实现都**必须**逐条断言全部向量；
任何一条对不上，就不是 AILog 兼容实现。

### 与 RFC 8785 (JCS) 的关系

本规范是 JCS 的**严格子集**：C1–C4、C7 与 JCS 一致，C5 比 JCS 更窄
（JCS 规定了浮点的规范化格式，本节直接禁止浮点）。
满足 C5 的 JCS 实现可直接用作 CJSON 实现。
